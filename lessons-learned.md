...
- **Invariant**: every Pigment target ships an explicit `main.swift`
...
- **Invariant**: every `NSTextView` in Pigment is built by the canonical
...
- **Invariant**: Pigment ships `NSOpenPanel` / `NSSavePanel` first.
...
- **Invariant**: in Pigment the build check MUST invoke at least one
...
- **Invariant**: Pigment's feature checks talk to the app via
...
- **Invariant**: each agent invocation in Pigment is wrapped by Go
...
- **Invariant**: every code-agent invocation in Pigment is sandwiched
...
- **Invariant**: in Pigment, no quality review starts until the
...
- **Always commit on a working tree.** The old loop refused to
  ...
  Pigment's orchestrator `git status --porcelain` checks both
  pre- and post-agent.
